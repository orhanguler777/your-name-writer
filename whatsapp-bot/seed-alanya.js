...
const departments = [
  { name: 'Afet İşleri ve Risk Yönetimi Müdürlüğü', description: 'Afet koordinasyonu, risk değerlendirmesi ve acil durum yönetimi' },
  { name: 'Bilgi İşlem Müdürlüğü', description: 'Bilişim altyapısı, yazılım ve donanım hizmetleri' },
  { name: 'Destek Hizmetleri Müdürlüğü', description: 'İdari destek, satın alma ve lojistik hizmetler' },
  { name: 'Dış İlişkiler Müdürlüğü', description: 'Uluslararası ilişkiler, kardeş şehirler ve protokol' },
  { name: 'Emlak ve İstimlak Müdürlüğü', description: 'Taşınmaz mal yönetimi, kamulaştırma ve kira işlemleri' },
  { name: 'Fen İşleri Müdürlüğü', description: 'Yol, asfalt, kaldırım, köprü ve altyapı çalışmaları' },
  { name: 'Gelirler Müdürlüğü', description: 'Belediye gelirleri, vergi ve harç tahsilat işlemleri' },
  { name: 'Gençlik ve Spor Hizmetleri Müdürlüğü', description: 'Gençlik merkezleri, spor tesisleri ve sportif etkinlikler' },
  { name: 'Hukuk İşleri Müdürlüğü', description: 'Hukuki danışmanlık, dava takibi ve sözleşme yönetimi' },
  { name: 'İklim Değişikliği ve Sıfır Atık Müdürlüğü', description: 'Çevre koruma, geri dönüşüm ve sıfır atık politikaları' },
  { name: 'İmar ve Şehircilik Müdürlüğü', description: 'İmar planları, yapı ruhsatları, kaçak yapı denetimi ve şehir planlama' },
  { name: 'İnsan Kaynakları ve Eğitim Müdürlüğü', description: 'Personel yönetimi, işe alım ve hizmet içi eğitimler' },
  { name: 'İşletme ve İştirakler Müdürlüğü', description: 'Belediye işletmeleri ve şirket iştirakleri yönetimi' },
  { name: 'Kentsel Dönüşüm Müdürlüğü', description: 'Kentsel dönüşüm projeleri ve riskli yapı tespit işlemleri' },
  { name: 'Kentsel Tasarım Müdürlüğü', description: 'Şehir estetiği, peyzaj düzenlemesi ve kentsel tasarım projeleri' },
  { name: 'Kırsal Hizmetler Müdürlüğü', description: 'Kırsal alan altyapısı, tarımsal destek ve köy hizmetleri' },
  { name: 'Kültür, Sanat ve Sosyal İşler Müdürlüğü', description: 'Kültürel etkinlikler, sanat faaliyetleri ve sosyal projeler' },
  { name: 'Kütüphane ve Müzeler Müdürlüğü', description: 'Halk kütüphaneleri ve müze yönetimi' },
  { name: 'Makina İkmal Bakım ve Onarım Müdürlüğü', description: 'Araç filosu bakımı, onarımı ve yedek parça tedariki' },
  { name: 'Mali Hizmetler Müdürlüğü', description: 'Bütçe hazırlama, muhasebe ve mali raporlama' },
  { name: 'Muhtarlık İşleri Müdürlüğü', description: 'Muhtarlık koordinasyonu ve mahalle talep yönetimi' },
  { name: 'Özel Kalem Müdürlüğü', description: 'Başkanlık sekretaryası, randevu ve protokol işleri' },
  { name: 'Park ve Bahçeler Müdürlüğü', description: 'Parklar, yeşil alanlar, ağaç budama, çiçeklendirme ve peyzaj' },
  { name: 'Plan ve Proje Müdürlüğü', description: 'Proje geliştirme, ihale hazırlık ve teknik planlama' },
  { name: 'Ruhsat ve Denetim Müdürlüğü', description: 'İşyeri açma ve çalışma ruhsatları, denetim faaliyetleri' },
  { name: 'Sosyal Hizmetler Müdürlüğü', description: 'Sosyal yardımlar, engelli hizmetleri ve toplumsal destek' },
  { name: 'Strateji Geliştirme Müdürlüğü', description: 'Stratejik planlama, performans yönetimi ve kurumsal gelişim' },
  { name: 'Teftiş Kurulu Müdürlüğü', description: 'İç denetim, teftiş ve soruşturma işlemleri' },
  { name: 'Temizlik İşleri Müdürlüğü', description: 'Çöp toplama, sokak temizliği, konteyner yönetimi ve atık bertarafı' },
  { name: 'Veteriner İşleri Müdürlüğü', description: 'Sokak hayvanları rehabilitasyonu, aşılama ve hayvan barınağı' },
  { name: 'Yapı Kontrol Müdürlüğü', description: 'Yapı denetimi, iskân ruhsatları ve bina güvenliği' },
  { name: 'Yazı İşleri Müdürlüğü', description: 'Resmi yazışmalar, meclis kararları ve evrak yönetimi' },
  { name: 'Zabıta Müdürlüğü', description: 'Çevre düzeni, gürültü denetimi, seyyar satıcı kontrolü ve kamu düzeni' },
];
...
async function seedAlanya() {
  ...
  if (oldNeighborhoods && oldNeighborhoods.length > 0) {
    const { error: delErr } = await supabase
      .from('neighborhoods')
      .delete()
      .in('id', oldNeighborhoods.map(n => n.id));
    if (delErr) {
  ...
  if (oldComplaints && oldComplaints.length > 0) {
    ...
    await supabase
      ...
      .delete()
      .in('complaint_id', oldComplaints.map(c => c.id));
    // Sonra şikayetleri sil
    const { error: delErr } = await supabase
      .from('complaints')
      .delete()
      .in('id', oldComplaints.map(c => c.id));
    if (delErr) {
