  ...
  for (const user of users.users) {
    if (user.email.endsWith('@alanya.bel.tr')) {
      ...
      try {
        ...
        if (updateAuthErr) {
          console.error(`⚠️ ${user.email} -> ${newEmail} (Auth) güncellenemedi:`, updateAuthErr.message);
          continue;
        }
        ...
        if (profileErr) {
          console.error(`⚠️ ${user.email} -> ${newEmail} (Profile) güncellenemedi:`, profileErr.message);
          continue;
        }
