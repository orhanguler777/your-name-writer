...
const STATUS_LABELS: Record<string, string> = {
  sahada: "Sahada", aktif: "Aktif", bakimda: "Bakımda", servis_disi: "Servis Dışı",
};
...
function daysBetween(a: string) { return Math.floor((Date.now() - new Date(a).getTime()) / 864e5); }

function Page() {
  const { data } = useQuery({
    queryKey: ["vehicles-all"],
    queryFn: async () => (await supabase.from("vehicles").select("*, departments(name)").order("plate_number")).data ?? [],
  });
  ...
  const uzun = data?.filter((v) => v.status === "bakimda" && v.maintenance_start_date && daysBetween(v.maintenance_start_date) > 7).length ?? 0;

  return (
    <div>
      <PageHeader title="Araç Bakım ve Saha Durumu" description="Belediye araçlarının anlık durumu ve bakım süreleri." />
      ...
      <Card>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Plaka</TableHead><TableHead>Tip</TableHead><TableHead>Müdürlük</TableHead>
              <TableHead>Durum</TableHead><TableHead>Bakım Başlangıç</TableHead>
              <TableHead>Süre</TableHead><TableHead>Sebep / Not</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {data?.map((v: any) => {
              const days = v.maintenance_start_date ? daysBetween(v.maintenance_start_date) : null;
              const alert = days !== null && v.status === "bakimda"
                ? days > 15 ? "kritik" : days > 7 ? "uyari" : null : null;
              return (
                <TableRow key={v.id}>
                  <TableCell className="font-medium">{v.plate_number}</TableCell>
                  <TableCell>{v.vehicle_type}</TableCell>
                  <TableCell>{v.departments?.name}</TableCell>
                  <TableCell><Badge className={STATUS_STYLE[v.status]}>{STATUS_LABELS[v.status]}</Badge></TableCell>
                  <TableCell className="text-xs">{v.maintenance_start_date ?? "—"}</TableCell>
                  <TableCell>
                    {days !== null && v.status !== "aktif" && v.status !== "sahada" && (
                      <Badge variant={alert === "kritik" ? "destructive" : "secondary"}
                        className={alert === "uyari" ? "bg-priority-medium/15 text-priority-medium" : ""}>
                        {days} gün
                      </Badge>
                    )}
                  </TableCell>
                  <TableCell className="text-xs max-w-xs">{v.maintenance_reason || v.notes || "—"}</TableCell>
                </TableRow>
              );
            })}
          </TableBody>
