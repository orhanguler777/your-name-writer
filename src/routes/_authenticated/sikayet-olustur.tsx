...
function Create() {
  ...
  const { data: neighborhoods } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("neighborhoods").select("id, name").order("name")).data ?? [],
  });
  ...
  const runClassify = async () => {
    if (form.complaint_text.trim().length < 10)
      return toast.error("Lütfen daha detaylı bir şikayet metni girin.");
    setLoading(true);
    try {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!ai) {
      try {
        ai = await classify({ data: { text: form.complaint_text } });
      } catch {}
    }
    const aiDept = ai?.department
      ? (departments?.find((d) => d.name === ai.department)?.id ?? null)
      : null;
    ...
    if (error) {
      setSubmitting(false);
      return toast.error("Şikayet oluşturulamadı", { description: error.message });
    }
    if (ai?.auto_response && data?.id) {
      await supabase.from("complaint_responses").insert({
        complaint_id: data.id,
        response_text: ai.auto_response,
        response_type: "otomatik",
      });
    }
    setSubmitting(false);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Şikayet veya talebinizi iletin, yapay zeka otomatik sınıflandırma yapacaktır."
      />
      ...
      <form onSubmit={submit} className="grid gap-4 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2 space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, citizen_email: e.target.value })}
              />
            </div>
            <div>
              <Label>Mahalle</Label>
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, neighborhood_id: v })}
              >
                <SelectTrigger>
          ...
          <div className="flex items-center gap-3 rounded-md border p-3">
            <Switch
              checked={form.wants_human_representative}
        ...
        <Card className="p-5 h-fit">
          ...
          {!aiResult ? (
            <p className="text-sm text-muted-foreground">
              Şikayet metnini yazdıktan sonra "AI ile Ön Sınıflandır" butonuna basın; kategori,
              öncelik ve önerilen müdürlük burada görünecek.
            </p>
          ) : (
            <div className="space-y-3 text-sm">
              <div>
                <div className="text-xs text-muted-foreground">Kategori</div>
                <div className="font-medium">{aiResult.category}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Önerilen Müdürlük</div>
                <div className="font-medium">{aiResult.department}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Öncelik</div>
                <div className="font-medium">{aiResult.priority}</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Güven Skoru</div>
                <div className="font-medium">%{Math.round((aiResult.confidence || 0) * 100)}</div>
              </div>
