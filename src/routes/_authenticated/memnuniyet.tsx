...
function Page() {
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center text-sm text-muted-foreground">Analizler yükleniyor...</div>
    );
  }
  ...
  const satisfactionRate =
    totalRated > 0
      ? (ratedComplaints.filter((c) => (c.satisfaction_score || 0) >= 4).length / totalRated) * 100
      : 0;
  ...
  const distributionData = [
    {
      name: "5 Yıldız (Çok İyi)",
      value: distributionMap[5],
      percent: totalRated > 0 ? (distributionMap[5] / totalRated) * 100 : 0,
    },
    {
      name: "4 Yıldız (İyi)",
      value: distributionMap[4],
      percent: totalRated > 0 ? (distributionMap[4] / totalRated) * 100 : 0,
    },
    {
      name: "3 Yıldız (Orta)",
      value: distributionMap[3],
      percent: totalRated > 0 ? (distributionMap[3] / totalRated) * 100 : 0,
    },
    {
      name: "2 Yıldız (Kötü)",
      value: distributionMap[2],
      percent: totalRated > 0 ? (distributionMap[2] / totalRated) * 100 : 0,
    },
    {
      name: "1 Yıldız (Çok Kötü)",
      value: distributionMap[1],
      percent: totalRated > 0 ? (distributionMap[1] / totalRated) * 100 : 0,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          label="Memnuniyet Seviyesi"
          value={averageScore >= 4 ? "Çok İyi" : averageScore >= 3 ? "Orta" : "Zayıf"}
          icon={Award}
        />
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="p-5">
          ...
          <div className="h-80">
            {departmentData.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground pt-32">
                Henüz puanlanmış bir müdürlük yok.
              </p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <Card className="p-5">
          <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
            <MapPin className="h-5 w-5 text-primary" /> Mahallelere Göre Memnuniyet Ortalaması (İlk
            10)
          </h3>
          <div className="h-80">
            {neighborhoodData.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground pt-32">
                Henüz puanlanmış bir mahalle yok.
              </p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
