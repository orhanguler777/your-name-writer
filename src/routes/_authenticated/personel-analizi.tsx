...
function Page() {
  ...
  att.forEach((a) => {
    dailyTrend[a.date] = (dailyTrend[a.date] ?? 0) + 1;
  });
  const trendData = Object.entries(dailyTrend)
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([date, count]) => ({ date: date.slice(5), count }));

  const toChart = (o: Record<string, number>) =>
    Object.entries(o)
  ...
  return (
    <div>
      <PageHeader
        title="Personel Giriş Çıkış Analizi"
        description="Devam kayıtları, geç girişler, fazla mesai ve eksik çıkışlar."
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5 mb-4">
        ...
        <KpiCard
          label="En Çok Geç Kalan"
          value={<span className="text-base">{topLateDept}</span>}
          icon={Bot}
        />
      </div>
      ...
      <Card className="p-5 mb-4 border-accent/40 bg-accent/5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-display font-semibold">AI İçgörü</h3>
        </div>
        <p className="text-sm leading-relaxed">
          Son 10 günde <strong>{topLateDept}</strong> müdürlüğünde geç giriş oranı diğer
          müdürlüklere göre daha yüksektir. Özellikle pazartesi ve cuma günleri yoğunluk
          görülmektedir. Vardiya planlaması kontrol edilebilir.
        </p>
      </Card>
