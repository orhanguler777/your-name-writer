...
export function AlanyaMap({ complaints }: AlanyaMapProps) {
  ...
  return (
    <>
      ...
      <Card
        className={`p-6 relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-950 text-slate-100 border-0 shadow-xl transition-all duration-300 ${
      ...
      >
        <div className="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <div>
            <h3 className="font-display font-semibold text-lg text-white">
              Alanya İnteraktif Coğrafi Şikayet Haritası
            </h3>
            <p className="text-xs text-slate-400">
              Canlı dalga efektli ısı noktaları ve mahalle bazlı detaylar (Marker'ların üzerine
              geliniz)
            </p>
          </div>
        ...
        <div
          className={`grid grid-cols-1 lg:grid-cols-4 gap-6 ${isFullscreen ? "flex-1 min-h-0" : ""}`}
        >
          {/* OpenLayers Harita Konteyneri */}
          <div
            className={`lg:col-span-3 rounded-xl overflow-hidden border border-slate-800 relative z-0 ${isFullscreen ? "h-full" : "h-[420px]"}`}
          >
          ...
          <div className="flex flex-col justify-between bg-slate-900/40 p-4 rounded-xl border border-slate-800">
            {activeStats ? (
                ...
                <div className="space-y-2">
                  <div
                    onClick={() =>
                      navigate({
                        to: "/sikayetler",
                        search: { neighborhood: activeStats.id, status: "all" } as any,
                      })
                    }
                  ...
                  <div
                    onClick={() =>
                      navigate({
                        to: "/sikayetler",
                        search: { neighborhood: activeStats.id, status: "active" } as any,
                      })
                    }
                  ...
                  <div
                    onClick={() =>
                      navigate({
                        to: "/sikayetler",
                        search: { neighborhood: activeStats.id, status: "cozuldu" } as any,
                      })
                    }
              ...
              <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 py-8">
                ...
                <p className="text-xs">
                  Detayları görmek için harita üzerindeki markerların üzerine geliniz.
                </p>
              </div>
            )}
